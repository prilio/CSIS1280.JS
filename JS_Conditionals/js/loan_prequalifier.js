...
function loanPrequalify(){
	...
	let workYears = document.getElementById("work_exp").value;  //VALUE RETORNA UM VALOR DE ATRIBUTO LET COM A VARIÁVEL WORKYEARS
	let curLoan = document.getElementById("cur_loan").value;
	let dtir = curLoan/salary;
	let output = document.getElementById("output"); //NÃO PRECISA DO VALUE, POIS NÃO RETORNARÁ STRING
	
	//amount prequalified 
	//amt = (DTIR-dtir)*salary
	let amtQualified = (DTIR-dtir)*parseFloat(salary) //PARSE ESTÁ CONVERTENDO O STRING DO SALARY PARA FLOAT NA EQUAÇÃO
	
	if(salary>=SALARY){
		if(workYears>=TENURE){
			if (dtir<=DTIR) {
				output.innerHTML ="Congratulations! You qualify for a loan. The maximum you can borrow is $" +amtQualified.toFixed(2); //TO FIXED É A QUANTIDADE DE CASAS DECIMAIS
			}
			else {
